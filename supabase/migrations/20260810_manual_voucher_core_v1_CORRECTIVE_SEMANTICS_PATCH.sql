-- RAWAEA Manual Stock Voucher Core V1 — CORRECTIVE SEMANTICS PATCH
-- Purpose: restore the original voucher movement semantics before application deployment.
-- No business/test data is inserted.
-- This patch replaces ONLY post_manual_stock_voucher_atomic.
-- IMPORTANT: execute ONCE in Production after explicit GO.
...
begin
  ...
  for v_effect in
    select * from jsonb_to_recordset(p_effects) as e(
      direction text,
      branch_id uuid,
      item_id uuid,
      item_code text,
  ...
  loop
    ...
    if p_operation = 'SEND' then
      ...
      if v_effect.direction = 'IN'
         ...
         and v_voucher.type in ('Transfer','DirectReturn') then
        null;
      else
  ...
  loop
    ...
    if p_operation = 'SEND' then
      ...
      if v_in_count <> 1 or v_out_count <> 0 or v_in_sum <= 0
         or v_in_sum > (v_detail.qty - v_detail.received_qty) then
        raise exception 'حركات الاستلام غير صالحة للصنف %', v_detail.item_code;
      end if;
    end if;
  end loop;
  ...
  if p_operation = 'SEND' then
    ...
    select count(*) into v_remaining_details
    from stock_voucher_details d
    where d.voucher_id = v_voucher.id
