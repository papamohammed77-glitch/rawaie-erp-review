-- RAWAEA Manual Stock Voucher Core V1 — FINAL DEPLOYMENT SCRIPT
-- Target branch: rescue/manual-vouchers-inventory-core
-- STATUS: Prepared for final Owner Read-Only Validation; NOT for Production execution yet.
--
-- This file consolidates the final intended definitions from:
--   20260810_manual_voucher_core_v1.sql
--   20260810_manual_voucher_core_v1_patch.sql
--   20260810_manual_voucher_core_v1_patch2.sql
--   20260810_manual_voucher_core_v1_patch3.sql
--
-- Execute this consolidated script ONCE only after the final read-only
-- validation package has passed and the owner explicitly approves deployment.
-- No test/business data is inserted by this migration.
--
-- Architectural contract:
--   Edge Function = authenticated/authorized API boundary.
--   Atomic RPC = authoritative inventory mutation boundary.
--   stock_branches = current stock state.
--   inventory_log = movement history.
--   Existing legacy send_stock_voucher_atomic is intentionally NOT dropped here.
...
begin
  ...
  for v_item in select value from jsonb_array_elements(p_items) loop
    ...
    values(
      v_voucher_id,v_item_id,v_item->>'itemCode',coalesce(nullif(v_item->>'itemName',''),v_item->>'itemCode'),
      coalesce(nullif(v_item->>'unit',''),'حبة'),(v_item->>'qty')::numeric,
      coalesce((v_item->>'unitPrice')::numeric,0),coalesce(v_item->>'notes','')
    );
  end loop;
...
begin
  ...
  loop
    if v_effect.direction not in ('OUT','IN') or v_effect.branch_id is null or v_effect.item_id is null or v_effect.item_code is null or v_effect.qty is null or v_effect.qty<=0 then
      raise exception 'بيانات حركة مخزنية غير صالحة';
    end if;

    select d.qty into v_detail_qty from stock_voucher_details d where d.voucher_id=v_voucher.id and d.item_id=v_effect.item_id and d.item_code=v_effect.item_code;
    ...
    if p_operation='SEND' then
      ...
      if v_effect.qty>v_detail_qty then raise exception 'الكمية المستلمة أكبر من كمية الإذن للصنف %',v_effect.item_code; end if;
    end if;
  end loop;

  for v_detail in select d.item_id,d.item_code,d.qty from stock_voucher_details d where d.voucher_id=v_voucher.id loop
    select coalesce(sum(case when e.direction='OUT' then e.qty else 0 end),0),coalesce(sum(case when e.direction='IN' then e.qty else 0 end),0),
           count(*) filter(where e.direction='OUT'),count(*) filter(where e.direction='IN'),count(*)
    ...
    if p_operation='SEND' then
      ...
      if v_voucher.type='Transfer' then
        if v_out_count<>0 or v_in_count<>1 or v_in_sum<=0 or v_in_sum>v_detail.qty then raise exception 'حركات استلام التحويل غير صالحة للصنف %',v_detail.item_code; end if;
      elsif v_voucher.type='DirectReturn' then
        if v_out_count<>1 or v_in_count<>1 or v_out_sum<=0 or v_out_sum<>v_in_sum or v_in_sum>v_detail.qty then raise exception 'حركات المرتجع المباشر غير صالحة للصنف %',v_detail.item_code; end if;
      end if;
    end if;
  end loop;
  ...
  loop
    ...
    values(p_company_id,v_log_code,current_date,p_voucher_code,v_effect.item_id,v_effect.item_code,
      coalesce((select d.item_name from stock_voucher_details d where d.voucher_id=v_voucher.id and d.item_id=v_effect.item_id limit 1),v_effect.item_code),
      v_voucher.type,v_qty,p_voucher_code,p_user_email);

    if p_operation='RECEIVE' and v_effect.direction='IN' then
      update stock_voucher_details set received_qty=v_qty where voucher_id=v_voucher.id and item_id=v_effect.item_id and item_code=v_effect.item_code;
    end if;
  end loop;

  update stock_vouchers set status=v_new_status,sent_date=case when p_operation='SEND' then now() else sent_date end,
