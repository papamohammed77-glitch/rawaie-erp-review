-- RAWAEA Manual Stock Voucher Core V1 — Reconciled Patch
-- Purpose: close the remaining lifecycle gap in RECEIVE handling before Production deployment.
-- This patch is intentionally additive/replacement-only: it replaces the atomic posting RPC.
-- It introduces cumulative partial-receive accounting and prevents over-receipt/replay.
-- No test/business data is inserted.
-- Execute only together with the FINAL deployment script, after Owner GO.
...
begin
  ...
  -- Validate every supplied effect against the voucher and current cumulative receipt.
  for v_effect in
  ...
  loop
    ...
    select d.qty, coalesce(d.received_qty,0)
      into v_detail_qty, v_received_qty
    ...
    v_remaining_qty := greatest(v_detail_qty-v_received_qty,0);
    ...
    if p_operation='SEND' then
      ...
      if v_remaining_qty<=0 then
        raise exception 'تم استلام كامل كمية الصنف سابقًا: %',v_effect.item_code;
      end if;
      if v_effect.qty>v_remaining_qty then
        raise exception 'الكمية المستلمة تتجاوز المتبقي للصنف %',v_effect.item_code;
      end if;
  ...
  -- Enforce exact SEND shape and cumulative RECEIVE shape per voucher detail.
  for v_detail in
    select d.item_id,d.item_code,d.qty,coalesce(d.received_qty,0) as received_qty
    from stock_voucher_details d where d.voucher_id=v_voucher.id
  loop
    select coalesce(sum(case when e.direction='OUT' then e.qty else 0 end),0),
    ...
    if p_operation='SEND' then
      if v_voucher.type in ('Transfer','SupplierReturn') then
        ...
        if v_out_count<>1 or v_in_count<>1 or v_out_sum<>v_detail.qty or v_in_sum<>v_detail.qty then
      ...
      if v_voucher.type='Transfer' then
        if v_out_count<>0 or v_in_count<>1 or v_in_sum<=0 or v_in_sum>greatest(v_detail.qty-v_detail.received_qty,0) then
          raise exception 'حركات استلام التحويل غير صالحة للصنف %',v_detail.item_code;
        end if;
      elsif v_voucher.type='DirectReturn' then
        if v_out_count<>1 or v_in_count<>1 or v_out_sum<=0 or v_out_sum<>v_in_sum
           or v_in_sum>greatest(v_detail.qty-v_detail.received_qty,0) then
  ...
  -- Apply the already validated effects under row locks.
  for v_effect in
    select * from jsonb_to_recordset(p_effects) as e(direction text,branch_id uuid,item_id uuid,item_code text,qty numeric)
    order by e.branch_id,e.item_id,e.direction
  loop
    select * into v_stock
    from stock_branches
    where branch_id=v_effect.branch_id and item_id=v_effect.item_id
    for update;
    ...
    values(
      p_company_id,v_log_code,current_date,p_voucher_code,v_effect.item_id,v_effect.item_code,
    ...
    if p_operation='RECEIVE' then
      if v_effect.direction='IN' then
        update stock_voucher_details
        set received_qty=coalesce(received_qty,0)+v_qty
        where voucher_id=v_voucher.id and item_id=v_effect.item_id and item_code=v_effect.item_code;
      end if;
    end if;
  end loop;
  ...
  if p_operation='SEND' then
    ...
    if not exists(
      select 1 from stock_voucher_details d
      where d.voucher_id=v_voucher.id
        and coalesce(d.received_qty,0)<d.qty
    ) then
  ...
  set status=v_new_status,
      ...
      received_date=case when p_operation='RECEIVE' and v_new_status='Received' then now() else received_date end,
      received_by=case when p_operation='RECEIVE' and v_new_status='Received' then p_user_email else received_by end
