-- RAWAEA Manual Stock Voucher Core V1 — RECONCILED DEPLOYMENT PATCH
-- Target: rescue/manual-vouchers-inventory-core
-- Production: NOT EXECUTED BY THIS FILE
-- Purpose: replace only post_manual_stock_voucher_atomic with the reconciled version.
--
-- Critical fixes:
-- 1) RECEIVE is cumulative through stock_voucher_details.received_qty.
-- 2) Partial RECEIVE keeps voucher status = Sent.
-- 3) Voucher becomes Received only when every detail is fully received.
-- 4) A RECEIVE can never exceed the remaining quantity.
-- 5) Replayed RECEIVE is blocked by status/remaining-quantity checks.
-- 6) SEND/RECEIVE effect cardinality and direction remain enforced centrally.
-- 7) No business/test data is inserted.
...
begin
  ...
  -- Validate every supplied effect against the voucher contract.
  for v_effect in
    select *
  ...
  loop
    ...
    if p_operation = 'SEND' then
      ...
      if v_effect.qty > (v_detail_qty - v_received_before) then
        raise exception 'الكمية المستلمة أكبر من الكمية المتبقية للصنف %', v_effect.item_code;
      end if;
  ...
  -- Validate effect cardinality per voucher detail.
  for v_detail in
  ...
  loop
    ...
    if p_operation = 'SEND' then
      ...
      -- A partial receive may contain only the items actually received now.
      if v_total_count = 0 then
        continue;
      end if;

      if v_voucher.type = 'Transfer' then
        if v_out_count <> 0 or v_in_count <> 1 or v_in_sum <= 0
           or v_in_sum > (v_detail.qty - v_detail.received_qty) then
          raise exception 'حركات استلام التحويل غير صالحة للصنف %', v_detail.item_code;
        end if;
      elsif v_voucher.type = 'DirectReturn' then
        if v_out_count <> 1 or v_in_count <> 1
           or v_out_sum <= 0 or v_out_sum <> v_in_sum
           or v_in_sum > (v_detail.qty - v_detail.received_qty) then
  ...
  -- Apply the movement atomically under row locks.
  for v_effect in
    select *
    from jsonb_to_recordset(p_effects) as e(
      direction text,
  ...
  loop
    ...
    values(
      p_company_id, v_log_code, current_date, p_voucher_code, v_effect.item_id,
      v_effect.item_code,
      coalesce((
        select d.item_name
        from stock_voucher_details d
    ...
    if p_operation = 'RECEIVE' and v_effect.direction = 'IN' then
      ...
      if not found then
        raise exception 'تعذر تحديث الكمية المستلمة للصنف %', v_effect.item_code;
      end if;
    end if;
  end loop;
  ...
  set status = v_new_status,
      ...
      received_date = case
        when p_operation = 'RECEIVE' and v_new_status = 'Received' then now()
        else received_date
      end,
      received_by = case
        when p_operation = 'RECEIVE' and v_new_status = 'Received' then p_user_email
        else received_by
      end
