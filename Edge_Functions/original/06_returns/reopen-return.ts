// reopen-return – إعادة فتح رانشيت مرتجع للتعديل (يخصم المخزون المُعاد مؤقتاً)
...
serve(async (req) => {
  ...
  try {
    ...
    if (rs.status !== "Returned") throw new Error("الرانشيت ليس في حالة مكتمل المرتجعات")
    if (rs.return_handler_id !== pubUser.id) throw new Error("هذا الرانشيت لا يخصك")

    var { data: details } = await supabase.from("run_sheet_details").select("item_code, item_id, qty_returned").eq("runsheet_id", rs.id)
    if (!details || !details.length) throw new Error("لا توجد تفاصيل للرانشيت")

    // خصم الكميات المُرتجعة سابقاً من المخزون (لتصفية أثرها مؤقتاً)
    for (var i = 0; i < details.length; i++) {
      ...
      if (qty > 0) {
        ...
        await supabase.from("stock_branches").update({ qty: Math.max(0, currentQty - qty) }).eq("branch_id", mainBranchId).eq("item_id", d.item_id)
      }
    }

    // إعادة الرانشيت إلى Returning مع الاحتفاظ بالكميات
    await supabase.from("runsheets").update({
      status: "Returning"
    }).eq("id", rs.id)

    return new Response(JSON.stringify({ success: true, msg: "تم إعادة فتح المرتجعات للتعديل." }), {
