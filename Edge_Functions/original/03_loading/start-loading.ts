// start-loading – بدء التحميل (مع منع تعدد المهام)
...
serve(async (req) => {
  ...
  try {
    ...
    // ✅ منع تعدد مهام التحميل
    var { data: activeRS } = await supabase.from("runsheets")
      .select("runsheet_code")
      .eq("status", "Loading")
      .eq("loader_id", pubUser.id)
      .maybeSingle()
    if (activeRS) throw new Error("لديك بالفعل رانشيت قيد التحميل: " + activeRS.runsheet_code + ". أنهِه أو ألغِه أولاً.")
    ...
    if (rs.status !== "Picked") throw new Error("لا يمكن بدء التحميل")

    await supabase.from("runsheets").update({
      status: "Loading", loader_id: pubUser.id, loader_start: new Date().toISOString()
    }).eq("runsheet_code", runsheet_code)

    return new Response(JSON.stringify({ success: true, msg: "تم بدء التحميل" }), {
      headers: { ...corsHeaders, "Content-Type": "application/json" }
    })
  } catch (error) {
    return new Response(JSON.stringify({ success: false, msg: error.message }), {
      status: 400,
      headers: { "Access-Control-Allow-Origin": req.headers.get("Origin") || "*", ...corsHeaders, "Content-Type": "application/json" }
    })
  }
})
